...
class BiomePresence():
    ...
    def init_gui(self):
        ...
        self.root.title("Noteab's Biome Macro (v1.5.5-patch1.1) (Idle)")
    ...
    def start_detection(self):
        if not self.detection_running:
            ...
            self.root.title("Noteab's Biome Macro (v1.5.5-patch1.1) (Running)")
    ...
    def stop_detection(self):
        if self.detection_running:
            ...
            self.start_time = None
            self.root.title("Noteab's Biome Macro (v1.5.5-patch1.1) (Stopped)")
        ...
        while self.detection_running:
            try:
                if not self.check_roblox_procs():
                    ...
                    self.root.title("Noteab's Biome Macro (v1.5.5-patch1.1) (Roblox Disconnected :c )")
                    ...
                    if self.config.get("auto_reconnect"):
                        ...
                        if private_server_link:
                            ...
                            for attempt in range(current_attempt, max_retries + 1):
                                ...
                                self.root.title("Noteab's Biome Macro (v1.5.5-patch1.1) (Reconnecting)")
    ...
    def reconnect_check_start_button(self):
        try:
            self.root.title("Noteab's Biome Macro (v1.5.5-patch1.1) (Reconnecting - In Main Menu)")
            reconnect_start_button = self.config.get("reconnect_start_button", [954, 876])
            max_clicks = 25
            failed_clicks = 0
    ...
    def send_webhook(self, biome, message_type, event_type):
        ...
        embed = {
            ...
            "color": biome_color,
            "footer": {
                "text": "Noteab's Biome Detection (v1.5.5-patch1.1)",
                "icon_url": icon_url
            },
            "fields": fields
        }
            ...
            "embeds": [
                {
                    ...
                    "color": color,
                    "footer": {
                        "text": "Noteab's Macro (v1.5.5-patch1.1)",
                        "icon_url": icon_url
                    }
                }
            ]
